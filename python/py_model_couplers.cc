...
void register_model_couplers(py::module & mod) {
  ...
  register_coupler_solid_contact<CouplerSolidContactInternodes>(
      mod, "CouplerSolidContactInternodes")
      .def(
          "getSolidMechanicsModel",
          [](CouplerSolidContactInternodes & self) -> decltype(auto) {
            return self.getSolidMechanicsModel();
          },
          py::return_value_policy::reference)
      .def(
          "initFull",
          [](CouplerSolidContactInternodes & self,
             const AnalysisMethod & analysis_method) {
            self.initFull(_analysis_method = analysis_method);
          },
          py::arg("_analysis_method") = _static);
}
