...
template <std::size_t N> struct Foreach {
  ...
  static constexpr inline auto find(Tuple &&tuple, V &&value) -> std::size_t {
...
auto make_named_tuple_no_decay(std::tuple<Names...> /*unused*/, Ts &&...args)
    -> decltype(auto) {
...
constexpr void foreach_impl(F &&func, Tuple &&tuple,
  ...
  (void)std::initializer_list<int>{
      (std::forward<F>(func)(std::get<Is>(std::forward<Tuple>(tuple))), 0)...};
}
...
template <class F, class Tuple, std::size_t... Is>
constexpr auto transform_named_impl(F &&func, Tuple &&tuple,
                                    std::index_sequence<Is...> && /*unused*/)
    -> decltype(auto) {
...
template <class Tuple, class... Vals, std::size_t... Is>
auto append_named_impl(std::index_sequence<Is...> && /*unused*/, Tuple &&tuple,
                       Vals &&...other_vals) -> decltype(auto) {
  return make_named_tuple_no_decay(
...
auto remove_impl(std::index_sequence<Is_before...> && /*unused*/,
                 std::index_sequence<Is_after...> && /*unused*/, Tuple &&tuple)
    -> decltype(auto) {
  return make_tuple_no_decay(
...
template <std::size_t nth, class Tuple, class Value, std::size_t... Is_before,
          std::size_t... Is_after>
auto replace_impl(std::index_sequence<Is_before...> && /*unused*/,
                  std::index_sequence<Is_after...> && /*unused*/, Tuple &&tuple,
                  Value &&value) -> decltype(auto) {
  return make_tuple_no_decay(
      std::get<Is_before>(std::forward<Tuple>(tuple))...,
...
auto replace_named_impl(std::index_sequence<Is_before...> && /*unused*/,
  ...
  return make_named_tuple_no_decay(
      std::tuple<tuple::tuple_name_tag_t<Is_before, tuple_type>...,
...
template <
    ...
    std::enable_if_t<is_named_tuple<std::decay_t<Tuple>>::value> * = nullptr>
auto remove(Tuple &&tuple) -> decltype(auto) {
  constexpr auto nth = tuple.template get_element_index(tuple::get<HashCode>());
