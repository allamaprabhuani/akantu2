...
template <hash_type HashCode> constexpr auto get() -> decltype(auto) {
  return make_named_tag<std::integral_constant<hash_type, HashCode>>();
}

template <hash_type HashCode, class Tuple>
constexpr auto get(Tuple &&tuple) -> decltype(auto) {
  return tuple.get(get<HashCode>());
}

template <hash_type HashCode, class Tuple> struct has_helper {
  using type =
      aka::bool_constant<std::decay_t<Tuple>::template has<HashCode>()>;
};

template <hash_type HashCode, class Tuple>
using has_t = typename has_helper<HashCode, Tuple>::type;

template <hash_type HashCode, class Tuple>
constexpr auto has(Tuple && /*tuple*/) -> bool {
  return has_t<HashCode, Tuple>::value;
}

template <hash_type HashCode, class Tuple> constexpr auto has() -> bool {
  return has_t<HashCode, Tuple>::value;
}
