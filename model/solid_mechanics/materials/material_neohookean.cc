...
/* -------------------------------------------------------------------------- */
MaterialNeohookean::MaterialNeohookean(SolidMechanicsModel & model, const MaterialID & id)  :
  Material(model, id) {
  AKANTU_DEBUG_IN();
...
Real MaterialNeohookean::celerity(const Element & elem ) {
  ...
  Real *strain_val = strain[elem.type]->values + elem.element * nb_quadrature_points * size_strain;
  Real F[3*3];

  //F is now the real F !!
  Real temprhot =0 ;
  Real C[3*3], Cinv[3*3],detC, defvol, p, traceCinv, coef,rhot;
...
void MaterialNeohookean::printself(std::ostream & stream, int indent) const {
  ...
  stream << space << "Material<_neohookean> [" << std::endl;
  if(!plain_stress)
