...
class DumperIOHelper::iterator_helper {
  typedef typename Array<T>::template const_iterator< R<T> > internal_iterator;
public:
...
template<class T>
class DumperIOHelper::iterator_helper<T, Matrix> {
...
template<class T>
class DumperIOHelper::PaddingHelper<T, Matrix> {
public:
  static inline Matrix<T> pad(const Matrix<T> & in, UInt padding_m, UInt padding_n, UInt nb_data) {
