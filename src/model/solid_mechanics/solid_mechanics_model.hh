...
namespace akantu {
  ...
  public:
    ...
    /// applya constant eigen_grad_u on all quadrature points of a given
    /// material
  ...
  protected:
    ...
    /// set the element_id_by_material and add the elements to the good
    /// materials
    virtual void assignMaterialToElements(
  ...
  protected:
    ...
    /// compute the external work (for impose displacement, the velocity should
    /// be given too)
  ...
  protected:
    ...
    void
    updateDataForNonLocalCriterion(ElementTypeMapReal & criterion) override;
  ...
  protected:
    void splitElementByMaterial(
        const Array<Element> & elements,
        std::vector<Array<Element>> & elements_per_mat) const;

    template <typename Operation>
    void splitByMaterial(const Array<Element> & elements,
                         Operation && op) const;
  ...
  public:
    ...
    [[deprecated(
        "Use getExternalForce instead of this function")]] Array<Real> &
    getForce() {
     ...
     * define a `potential` energy type. For additional energy types, see
    ...
    AKANTU_GET_MACRO_BY_ELEMENT_TYPE_CONST(MaterialByElement, material_index,
                                           UInt);
    // AKANTU_GET_MACRO_BY_ELEMENT_TYPE(MaterialByElement, material_index,
    // UInt);
  ...
  protected:
    ...
    /// displacements array at the previous time step (used in finite
    /// deformation)
