...
inline std::ostream &operator<<(std::ostream &stream, const Resolution &_this) {
...
#define RESOLUTION_DEFAULT_PER_DIM_ALLOCATOR(id, res_name)                     \
  [](UInt dim, const ID &, ContactMechanicsModel &model, const ID &id)         \
      -> std::unique_ptr<Resolution> {                                                                                           \
