...
CohesiveElementInserterHelper::CohesiveElementInserterHelper(
  ...
  for (auto && facet_to_double : potential_facets_to_double) {
    ...
    auto & facets_to_elements = mesh_accessor.getSubelementToElement(
...
void CohesiveElementInserterHelper::updateElementalConnectivity(
  ...
  for (const auto & element : element_list) {
    ...
    auto && connectivity = mesh_accessor.getConnectivity(element);
    ...
    if (element.kind() == _ek_cohesive) {
      ...
      for (const auto & [n, facet] : enumerate(facets)) {
        ...
        auto begin =
            connectivity.begin() + static_cast<Int>(n * facet_nb_nodes);
...
template <Int dim> void CohesiveElementInserterHelper::findSubfacetToDouble() {
  ...
  for (auto f : arange(2)) {
    for (auto && facet_to_double : make_view(facets_to_double, 2)) {
      ...
      for (const auto & subfacet : subfacets_to_facet) {
        if (subfacet == ElementNull) {
          continue;
        }
        ...
        if (dim == spatial_dimension - 2) {
          ...
          for (const auto & subsubfacet : subsubfacets_to_subfacet) {
...
void CohesiveElementInserterHelper::doubleNodes(
  ...
  MeshAccessor accessor(mesh);
  auto & position = accessor.getNodes();
  auto spatial_dimension = mesh.getSpatialDimension();
  ...
  for (auto && [n, old_node] : enumerate(old_nodes)) {
...
void CohesiveElementInserterHelper::doublePointFacet() {
  ...
  for (auto ghost_type : ghost_types) {
    for (auto facet_type : nb_new_facets.elementTypes(
      ...
      auto & connectivities =
          mesh_facets_accessor.getConnectivity(facet_type, ghost_type);
  ...
  for (auto facet_to_double : make_view(facets_to_double, 2)) {
    ...
    auto & facet_connectivities = mesh_facets_accessor.getConnectivity(
        old_facet.type, old_facet.ghost_type);
    ...
    auto && segment_conectivity = mesh_accessor.getConnectivity(element);
