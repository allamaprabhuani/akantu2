...
data stored per element type in a \code{ElementTypeMapArray} is composed by as
...
\code{Array}s of 2 components, or the $2^{nd}$ order tensors with $2\times2$ components.  
This memory can be dealt with \code{addDumpFieldArray}\index{I\/O!addDumpFieldArray} which always dumps
\code{Arrays} with 3 components or \code{addDumpFieldTensor}\index{I\/O!addDumpFieldTensor} which dumps $2^{nd}$
...
\begin{tabular}{llll}
  ...
  key          &    type      & support \\
  \midrule
  displacement & Array<Real> & nodes  \\
  velocity     & Array<Real> & nodes  \\
  acceleration & Array<Real> & nodes  \\
  force	       & Array<Real> & nodes  \\
  residual     & Array<Real> & nodes  \\
  boundary     & Array<bool> & nodes  \\
  mass         & Array<Real> & nodes  \\
...
\begin{cpp}
  Array<T> arr(nb_nodes, nb_component);
  Field field = new DumperIOHelper::NodalField<T>(arr));
