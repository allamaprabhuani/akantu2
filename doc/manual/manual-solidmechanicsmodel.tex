\chapter{Solid MechanicsModel\index{SolidMechanicsModel}\label{sect:smm}}
...
function \code{createBoundariesFromMeshData} is called.  This function
...
scheme is selected using the \code{\_explicit\_dynamic} constant.
...
wave) needs to travel the characteristic length of the mesh.  However,
note that if the time step of the computation is equal to the stable
time step, the simulation remains unstable. Therefore, it is necessary
to impose a time step that is smaller than the stable time step, for
...
\item \code{model.explicitPred()} allows to compute the displacement
  ...
  $\vec{\dot{u}_{n+1/2}}$, which will be used later in the method
...
formulation is used, the tangent matrix has to be computed. Every
...
user-defined material for the  simulation. These materials are
 referred to as local materials since they are local to the example
of the user and not part of the \akantu library.  To define a new local
material, two files (\code {material\_XXX.hh} and
\code{material\_XXX.cc}) have to be provided where \code{XXX} is the
name of the new material. Define the interface of your custom material
in \code{material\_XXX.hh} and its implementation in
\code{material\_XXX.cc}. The interface has to be included in you example:
\begin{cpp}
#include "material\_XXXX.hh"
\end{cpp}
 The new law must inherit from the
\code{Material} class or any other existing material class. The
declaration of the parent class needs therefore to be included in the
\code{material\_XXX.hh} file:
\begin{cpp}
#include "material.hh"
\end{cpp}
The following methods have to be redefined for the new constitutive
law:
\begin{cpp}
void initMaterial();
...
\end{cpp}
These methods are explained in detail at the end of this section.  For
each material, the minimum required parameters have to be defined in a
material file.  For existing materials, as mentioned in
Section~\ref{sect:smm:CL}, by default, the materials are initialized
inside the method \code{initFull}. If a local material should be used,
the initialization of the materials has to be postponed until the
local material is registered in the model. Therefore, the model is
...
constructor of the class with the method
...
can be registered in the model by calling the method
\code{registerNewCustomMaterial}:
\begin{cpp}
model.registerNewCustomMaterials<XXX>("name_of_local_material");
\end{cpp} In the following, the methods that have to be redefined
...
\item \code{initMaterial}:~ This method is called after the material
  ...
  calculated in this method. For example, the Lamé's constants of
