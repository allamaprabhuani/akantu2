...
\item[boundary] contains a boolean value for each degree of freedom
  ...
  \textbf{boundary} value of a degree of freedom to \code{true}.  A
  ...
  \textbf{boundary} value is set to \code{false}. For the remaining
...
\end{cpp} 
This function initializes the internal arrays and sets them
...
\end{cpp} 
\index{Constitutive\_laws} where \emph{constitutive\_law} is
...
\item \code{computeStress}:~ In this function, the stresses are
  ...
  \begin{equation}
    \label{eqn:smm:constitutive_elastic} \mat{\sigma }
...
\item \code{getStableTimeStep}:~ The stable time step should be
  ...
  the value of this velocity should be defined in this function for
  each new material.
