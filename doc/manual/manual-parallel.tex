...
computation, the mesh is partitioned. We assume that a \code{Mesh} object
...
only partitioner available is \code{MeshPartitionScotch}, which implements the
function \code{partitionate} using the \textbf{Scotch}\cite{scotch} program.
...
\code{SolidMechanicsModel} through the \code{initParallel} function.  Thus,
after creating a \code{SolidMechanicsModel} with our mesh as the initial
parameter, the \code{initParallel()} method must be called with the partition
as a parameter.
...
context, can be found in \code{\examplesdir/parallel\_2d}.
