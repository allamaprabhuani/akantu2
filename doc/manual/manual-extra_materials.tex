...
\subsubsection{Small-deformation Plasticity}\index{Material!Small-deformation Plasticity}
...
Here, we summarize the implementation procedures for the small-deformation plasticity with linear isotropic hardening. We use an implicit integration technique called \textit{the radial return method} to obtain the plastic multiplier. This method has an advantage of being unconditionally stable, however, the accuracy remains dependent on the step size. The plastic parameters to indicate in the material file are: \code{$\sigma_y$} (Yield stress) and \code{h} (Hardening modulus). In addition, the elastic parameters need to be defined as previous mentioned: \code{E} (Young's modulus), \code{nu} (Poisson's ratio).
