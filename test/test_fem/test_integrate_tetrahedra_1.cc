...
int main(int argc, char *argv[]) {
  ...
  fem->interpolateOnQuadraturePoints(const_val, val_on_quad, 2, _tetrahedra_1);
  //integrate function on elements
  akantu::Vector<akantu::Real> int_val_on_elem(0, 2, "int_val_on_elem");
  fem->integrate(val_on_quad,int_val_on_elem,2,_tetrahedra_1);
  // get global integration value
  Real value[2] = {0,0};
  std::ofstream my_file("out.txt");
  my_file << int_val_on_elem << std::endl;
  for (UInt i = 0; i < fem->getMesh().getNbElement(_tetrahedra_1); ++i) {
