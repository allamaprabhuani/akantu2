...
int main(int argc, char *argv[])
{
  ...
  double * surf_id = new double [my_mesh.getSurfaceId(_segment_2).getSize()];
  for (UInt i = 0; i < my_mesh.getSurfaceId(_segment_2).getSize(); ++i)
    surf_id[i] = (double)my_mesh.getSurfaceId(_segment_2).values[i];
